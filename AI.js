...
function getRealScore(leftRow, leftCol, rightRow, rightCol, val) {
    ...
    if ((qiziS[leftRow][leftCol].value == 0 || qiziS[leftRow][leftCol].value == val) && (qiziS[rightRow][rightCol].value == 0 || qiziS[rightRow][rightCol].value == val)) {
        return true;
    } else {
        return false;
    }
...
function analysisObliUpLine(row ,col, val) {//斜上方向分析
    ...
    for (var i = 0 ; i < 5 ; i ++) {
        ...
        var arr = [];
        for (var j = 0 ; j < 5 ; j ++) {
            arr.push(qiziS[row + 4 - i - j][col - 4 + i + j]);
        }
...
function analysisObliDownLine(row ,col, val) {//斜下方向分析
    ...
    for (var i = 0 ; i < 5 ; i ++) {
        ...
        var arr = [];
        for (var j = 0 ; j < 5 ; j ++) {
            arr.push(qiziS[row - i + j][col - i + j]);
        }
        var score = getScore(arr, val);
...
function analysisPowerScore(x, y) {
    ...
    power[0] = x - 1 < 0 || y - 1 < 0 ? 0 : qiziS[x - 1][y - 1].value;
    power[1] = x - 1 < 0 ? 0 : qiziS[x - 1][y].value;
    power[2] = x - 1 < 0 || y + 1 > 14 ? 0 : qiziS[x - 1][y + 1].value;
    power[3] = y - 1 < 0 ? 0 : qiziS[x][y - 1].value;
    power[4] = y + 1 > 14 ? 0 : qiziS[x][y + 1].value;
    power[5] = x + 1 > 14 || y - 1 < 0 ? 0 : qiziS[x + 1][y - 1].value;
    power[6] = x + 1 > 14 ? 0 : qiziS[x + 1][y].value;
    power[7] = x + 1 > 14 || y + 1 > 14 ? 0 : qiziS[x + 1][y + 1].value;
...
function aiAction() {
    ...
    for (var i = 0 ; i < qiziS.length ; i ++) {
        for (var j = 0 ; j < qiziS[i].length ; j ++) {
            ...
            if (debug) {
                qiziS[i][j].innerHTML = "" + defenseScore;
            }
